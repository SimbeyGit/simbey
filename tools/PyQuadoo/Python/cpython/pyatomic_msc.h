...
static __inline char _InterlockedExchange8 (volatile char* dest, char value)
{
    __asm
	{
        mov edx, dest     ; Load pointer
        mov al, value     ; Load new value into AL
        xchg al, byte ptr [edx] ; Atomically exchange AL with *dest
        ; AL now contains the original value at *dest
    }
    // Not directly returned from inline __asm block — compiler limitation
    // So we use a temp:
    char original;
    __asm { mov original, al }
    return original;
}

static __inline short _InterlockedExchange16 (volatile short* dest, short value)
{
    short original;
    __asm
	{
        mov edx, dest     ; Load pointer
        mov ax, value     ; Load new value into AX
        xchg ax, word ptr [edx] ; Atomically exchange AX with *dest
        mov original, ax  ; AX now has original value at *dest
    }
    return original;
}

// 8-bit atomic compare exchange (MSVC lacks this; inline asm required)
static __inline char _InterlockedCompareExchange8 (volatile char* dest, char exch, char comparand)
{
    __asm
	{
        mov edx, dest
...
static __inline char _InterlockedExchangeAdd8 (volatile char* ptr, char val)
{
    ...
    do {
        old = *ptr;
        expected = old;
    } while (_InterlockedCompareExchange8(ptr, old + val, expected) != expected);
    return old;
}

// 16-bit atomic exchange add (simulate with CAS loop)
static __inline short _InterlockedExchangeAdd16 (volatile short* ptr, short val)
{
    short old, expected;
    do {
        old = *ptr;
        expected = old;
    } while (InterlockedCompareExchange16(ptr, old + val, expected) != expected);
    return old;
}
...
#else
    return (int32_t)InterlockedExchangeAdd((volatile long *)obj, (long)value);
#endif
}
...
_Py_atomic_compare_exchange_int32(int32_t *obj, int32_t *expected, int32_t value)
{
    ...
    int32_t initial = (int32_t)_InterlockedCompareExchange(
                                       ...
                                       (long)*expected);
#else
    int32_t initial = (int32_t)InterlockedCompareExchange(
                                       (volatile long *)obj,
                                       (long)value,
                                       (long)*expected);
#endif
...
_Py_atomic_compare_exchange_int64(int64_t *obj, int64_t *expected, int64_t value)
{
    ...
    int64_t initial = (int64_t)_InterlockedCompareExchange64(
                                       ...
                                       (__int64)*expected);
#else
    int64_t initial = (int64_t)InterlockedCompareExchange64(
                                       (volatile __int64 *)obj,
                                       (__int64)value,
                                       (__int64)*expected);
#endif
...
{
    void *initial = InterlockedCompareExchangePointer(
                                       (void**)obj,
                                       value,
                                       *(void**)expected);
...
#else
    return (int32_t)InterlockedExchange((volatile long *)obj, (long)value);
#endif
}
...
#else
    return (void*)InterlockedExchangePointer((void * volatile *)obj, (void *)value);
#endif
}
