...
class CAccessibleNetDoc : public IAccessibleNetDoc
{
private:
	ULONG m_cRef;
...
protected:
	...
	HMODULE m_hOleAcc;
	HRESULT (WINAPI* lpfnAccessibleObjectFromWindow)(HWND,DWORD,REFIID,LPVOID);

public:
	CAccessibleNetDoc (CNeuralDocument* lpDoc, CGraphCtrlAdapter* pGraphAdapter);
	virtual ~CAccessibleNetDoc ();

	HRESULT Initialize (VOID);
	VOID AttachNet (INeuralNet* lpNet);
	...
	// IAccessibleNetDoc
	HRESULT NotifyAccEvent (DWORD dwEvent, INetDocObject* lpObject);
	VOID GraphToClient (FLOAT gx, FLOAT gy, LONG* px, LONG* py);
	FLOAT GetGraphScale (VOID);
	HRESULT ScreenToGraph (POINT& ptScreen, FLOAT& fxGraph, FLOAT& fyGraph);
