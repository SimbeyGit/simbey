	...
	)
{
	...
	do
	{
		...
		{
			itemList = reinterpret_cast<SCRIPT_ITEM*>(__realloc(itemList, allocLen * sizeof(SCRIPT_ITEM)));
	...
	while(i < itemCount)
	{
		...
		{
			mergedAllocLen += 16;
			mergedList = reinterpret_cast<ITEM_RUN*>(__realloc(mergedList, mergedAllocLen * sizeof(ITEM_RUN)));
		}
...
BOOL ShapeAndPlaceItemRun(USPDATA *uspData, ITEM_RUN *itemRun, HDC hdc, WCHAR *wrunstr)
{
	...
	do
	{
		...
		if(uspData->glyphCount + reallocSize >= uspData->glyphAllocLen)
		{
			...
			uspData->glyphList	 = reinterpret_cast<WORD*>(__realloc(uspData->glyphList, uspData->glyphAllocLen * sizeof(WORD)));
			uspData->offsetList  = reinterpret_cast<GOFFSET*>(__realloc(uspData->offsetList, uspData->glyphAllocLen * sizeof(GOFFSET)));
			uspData->widthList	 = reinterpret_cast<INT*>(__realloc(uspData->widthList, uspData->glyphAllocLen * sizeof(int)));
			uspData->svaList     = reinterpret_cast<SCRIPT_VISATTR*>(__realloc(uspData->svaList, uspData->glyphAllocLen * sizeof(SCRIPT_VISATTR)));
		}
	...
	)
{
	...
	if(uspFontList == 0)
	{
		uspData->defaultFont.hFont = NULL;
	...
	{
		uspData->bidiLevels	= reinterpret_cast<BYTE*>(__realloc(uspData->bidiLevels, uspData->itemRunAllocLen * sizeof(BYTE)));
		uspData->visualToLogicalList = reinterpret_cast<INT*>(__realloc(uspData->visualToLogicalList, uspData->itemRunAllocLen * sizeof(int)));
	}
	...
	if(uspData->stringAllocLen < wlen)
	{
		...
		uspData->clusterList	= reinterpret_cast<WORD*>(__realloc(uspData->clusterList,	wlen * sizeof(WORD)));
		uspData->attrList		= reinterpret_cast<ATTR*>(__realloc(uspData->attrList,	wlen * sizeof(ATTR)));
		uspData->breakList		= reinterpret_cast<SCRIPT_LOGATTR*>(__realloc(uspData->breakList,	wlen * sizeof(SCRIPT_LOGATTR)));
	}
...
{
	USPDATA* uspData = reinterpret_cast<USPDATA*>(__malloc(sizeof(USPDATA)));
...
VOID WINAPI UspFree(USPDATA *uspData)
{
	if(uspData)
	{
		...
		__free(uspData->glyphList);
		__free(uspData->svaList);
		__free(uspData->widthList);
		__free(uspData->offsetList);
		
		// free the item-run buffers
		__free(uspData->itemRunList);
		__free(uspData->visualToLogicalList);
		__free(uspData->bidiLevels);
		__free(uspData->tempItemList);

		// free the logical character-buffers
		__free(uspData->clusterList);
		__free(uspData->breakList);
		__free(uspData->attrList);
