...
const WCHAR c_wzTextEditorClass[] = L"TextEditorCls";
...
HRESULT CTextEditor::Register (HINSTANCE hInstance)
{
	WNDCLASSEX wnd = {0};
	...
	wnd.lpszClassName = c_wzTextEditorClass;

	return RegisterClass(&wnd,NULL);
}

HRESULT CTextEditor::Unregister (HINSTANCE hInstance)
{
	return UnregisterClass(c_wzTextEditorClass, hInstance);
}

HRESULT CTextEditor::Initialize (HWND hwndParent, const RECT& rcSite, INT nTabWidth)
{
	HRESULT hr;

	m_pTextDoc = __new CTextDocument(nTabWidth);
	CheckAlloc(m_pTextDoc);
	Check(m_pTextDoc->Initialize());

	Check(Create(0, WS_VSCROLL | WS_HSCROLL | WS_CHILD | WS_VISIBLE, c_wzTextEditorClass, NULL, rcSite.left, rcSite.top, rcSite.right - rcSite.left, rcSite.bottom - rcSite.top, hwndParent, SW_NORMAL));
...
{
	HRESULT hr;
	TStackRef<CTextDocument> srDocument(m_pTextDoc);

	if(srDocument->m_seq.is_prepared())
	{
		srDocument.Release();
		srDocument.Attach(__new CTextDocument(m_pTextDoc->m_nTabWidth));
		CheckAlloc(srDocument);
		Check(srDocument->Initialize());
	}
...
{
	return m_pTextDoc->m_seq.canundo();
}

bool CTextEditor::CanRedo (VOID)
{
	return m_pTextDoc->m_seq.canredo();
}
...
{
	m_nSelectionStart = 0;
	m_nSelectionEnd   = m_pTextDoc->m_seq.size();
	m_nCursorOffset   = m_nSelectionEnd;
...
BOOL CTextEditor::ForwardDelete (VOID)
{
	...
	else
	{
		...
		size_w n;
		m_pTextDoc->m_seq.render(m_nCursorOffset, tmp, 2, &n);
...
CTextDocument* CTextEditor::GetTextDocument (VOID)
{
	return m_pTextDoc;
}

VOID CTextEditor::SetTextDocument (CTextDocument* pDocument)
{
...
ULONG CTextEditor::GetText (PWSTR pwzDest, ULONG nStartOffset, ULONG nLength)
{
	...
	if(1 < nLength)
	{
		CTextIterator itor = m_pTextDoc->Iterate(nStartOffset);
...
USPCACHE* CTextEditor::GetUspCache (HDC hdc, ULONG nLineNo, ULONG* nOffset)
{
	...
	if(nLineNo < m_pTextDoc->LineCount() && SUCCEEDED(m_stmTextBuff.TWriteAdvance(&buff, cbAdvance)) && SUCCEEDED(m_stmAttrBuff.TWriteAdvance(&attr, cbAdvance)))
	{
		...
		off_chars = m_pTextDoc->m_aLines[nLineNo];
		m_pTextDoc->m_seq.render(off_chars, buff, len, &cchCopied);
	}
	else
	{
		len = 0;
	...
	//
	int				tablist[]		= { m_pTextDoc->m_nTabWidth };
...
int CTextEditor::ApplyTextAttributes (ULONG nLineNo, ULONG nOffset, ULONG &nColumn, PWSTR szText, int nTextLen, ATTR* attr)
{
	...
	for(i = 0; i < nTextLen; i++)
	{
		...
		if(szText[i] == '\t')
			nColumn += m_pTextDoc->m_nTabWidth - (nColumn % m_pTextDoc->m_nTabWidth);
		else
			nColumn += 1;
	}
...
VOID CTextEditor::SetupScrollbars ()
{
	...
	si.nMin  = 0;
	si.nMax  = m_pTextDoc->m_cchLongestLine - 1;	// total number of lines in file
	...
	m_nHScrollMax = m_pTextDoc->m_cchLongestLine - m_nWindowColumns;
}
...
bool CTextEditor::PinToBottomCorner ()
{
	...
	if(static_cast<size_w>(m_nHScrollPos + m_nWindowColumns) > m_pTextDoc->m_cchLongestLine)
	{
		m_nHScrollPos = m_pTextDoc->m_cchLongestLine - m_nWindowColumns;
		repos = true;
	}
...
LONG CTextEditor::InvalidateRange (ULONG nStart, ULONG nFinish)
{
	...
	if(lineno < m_nVScrollPos)
	{
		...
		itor   = m_pTextDoc->IterateLine(lineno, &off_chars, &len_chars);
		start  = off_chars;
	}
	else
	{
...
BOOL CTextEditor::MouseCoordToFilePos (int mx, int my, ULONG* pnLineNo, ULONG* pnFileOffset, int* psnappedX)
{
	...
	if(nLineNo >= m_pTextDoc->LineCount())
	{
		...
		off_chars = m_pTextDoc->m_seq.size();
	}
	...
	// return coords!
	CTextIterator itor = m_pTextDoc->IterateLine(nLineNo, &off_chars, &cchLine);
	*pnLineNo		= nLineNo;
...
{
	m_nCursorOffset = m_pTextDoc->m_seq.size();
}
...
LRESULT CTextEditor::SizeEditor (UINT nFlags, int width, int height)
{
	...
	m_nWindowColumns = min((width - margin)	/ m_nFontWidth,  static_cast<INT>(m_pTextDoc->m_cchLongestLine));
...
LRESULT CTextEditor::OnLButtonDown (UINT nFlags, int mx, int my)
{
	...
	else
	{
		...
		m_nSelectionStart = m_pTextDoc->m_aLines[nLineNo];
