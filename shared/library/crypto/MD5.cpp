...
CMd5::~CMd5 ()
{
	SecureZeroMemory(&m_ctxKey, sizeof(MD5_CTX));
}
...
VOID CMd5::AddData (const BYTE* pcbData, UINT cData)
{
	Update(&m_ctxKey,pcbData,cData);
}

UINT CMd5::GetDigestSize (VOID)
{
	return 16;
}

VOID CMd5::GetDigest (LPBYTE lpDigest)
{
	MD5_CTX Key;
	CopyMemory(&Key, &m_ctxKey, sizeof(MD5_CTX));
	CompleteKey(&Key, lpDigest);
}

UINT CMd5::GetHexKeySize (VOID)
{
	return 32;
}

VOID CMd5::GetHexKey (LPSTR lpszKey)
{
	BYTE Digest[16];
	GetDigest(Digest);
...
VOID CMd5::Update (LPMD5_CTX lpKey, const BYTE* pcbData, UINT cData)
{
...
VOID CMd5::Transform (ULONG state[4], const BYTE block[64])
{
...
VOID CMd5::Encode (LPBYTE output, ULONG* input, UINT len)
{
...
VOID CMd5::Decode (ULONG* output, const BYTE* input, UINT len)
{
...
VOID CMd5::CompleteKey (LPMD5_CTX lpKey, BYTE Digest[16])
{
	BYTE bits[8];
	ULONG index, padLen;
	...
	SecureZeroMemory(lpKey, sizeof(MD5_CTX));
}